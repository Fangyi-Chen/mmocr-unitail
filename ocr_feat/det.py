...
def main():
    ...
    for t in train_or_test:
        classes = os.listdir(osp.join(args.img_root, t))
        progressbar = ProgressBar(task_num=len(classes))
        for cls in classes:
            progressbar.update()
            for im in os.listdir(osp.join(args.img_root, t, cls)):
                img_path = osp.join(args.img_root, t, cls, im)
                ...
                # save result
                out_txt_dir_ = osp.join(out_txt_dir, t, cls)
                mmcv.mkdir_or_exist(out_txt_dir_)
                save_results(result, out_txt_dir_, img_name, score_thr=args.score_thr)
                # show result
                out_vis_dir_ = osp.join(out_vis_dir, t, cls)
                mmcv.mkdir_or_exist(out_vis_dir_)
                out_file = osp.join(out_vis_dir_, img_name)
                kwargs_dict = {
